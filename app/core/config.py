...
class Settings(BaseSettings):
    ...
    VISUAL_PHOTOS_SAMPLE_SIZE: int = Field(
        default=3
    )  # Number of photos to analyze per listing
    ...
    ENABLE_AUTO_INGESTION: bool = Field(
        default=True
    ...
    REALTOR_SEARCH_URL: str = Field(
        default="https://www.realtor.com/realestateandhomes-search/San-Francisco_CA"
    )
    CRAIGSLIST_SEARCH_URL: str = Field(
        default="https://sfbay.craigslist.org/search/rea"
    )
    ...
    SEARCH_PRICE_MAX: Optional[int] = Field(
        default=None
    )  # Maximum property price (None = no limit)

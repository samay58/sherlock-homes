...
def initial_setup():
    ...
    if is_sqlite:
        ...
        if settings.RUN_DB_MIGRATIONS_ON_STARTUP:
            try:
                ...
                logger.error(f"Error applying migrations at startup: {e}", exc_info=True)
        else:
            logger.info("RUN_DB_MIGRATIONS_ON_STARTUP is false; skipping DB migrations.")
    ...
    if os.getenv("ZENROWS_API_KEY") and settings.ENABLE_AUTO_INGESTION:
        ...
        logger.info(f"Scheduled ingestion job to run every {settings.INGESTION_INTERVAL_HOURS} hours.")
        scheduler.start()
    else:
        if not os.getenv("ZENROWS_API_KEY"):
            logger.warning("ZENROWS_API_KEY not set; skipping ingestion scheduler setup.")
        else:
            logger.info("Automatic ingestion is disabled via ENABLE_AUTO_INGESTION setting.")

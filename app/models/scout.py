...
from sqlalchemy import Integer, String, Float, ForeignKey, Column, Boolean, JSON, Text, DateTime
...
class Scout(Base):
    ...
    # Link to criteria
    criteria_id = Column(Integer, ForeignKey("criteria.id", ondelete="SET NULL"), nullable=True)
    ...
    search_neighborhoods = Column(JSON, nullable=True)  # Specific neighborhoods to focus on
    ...
    seen_listing_ids = Column(JSON, nullable=True, default=list)  # List of listing IDs already alerted
    
    # Scout learning/feedback
    positive_feedback_listings = Column(JSON, nullable=True, default=list)  # Liked listings
    negative_feedback_listings = Column(JSON, nullable=True, default=list)  # Rejected listings
    ...
    scout_runs = relationship("ScoutRun", back_populates="scout", cascade="all, delete-orphan")

...
def create_scout(scout_data: ScoutCreate, db: Session = Depends(get_db)):
...
async def run_scout(
    scout_id: int, background_tasks: BackgroundTasks, db: Session = Depends(get_db)
):
...
def get_scout_runs(scout_id: int, limit: int = 10, db: Session = Depends(get_db)):
    """Get recent runs for a scout."""
    runs = (
        db.query(ScoutRun)
        .filter(ScoutRun.scout_id == scout_id)
...
def get_scout_matches(scout_id: int, db: Session = Depends(get_db)):
    ...
    latest_run = (
        db.query(ScoutRun)
        .filter(ScoutRun.scout_id == scout_id, ScoutRun.status == "completed")
        .order_by(ScoutRun.started_at.desc())
        .first()
    )
...
def submit_feedback(
    scout_id: int, feedback: ScoutFeedback, db: Session = Depends(get_db)
):

...
def create_sample_listings(db: Session):
    ...
    for listing in sample_listings:
        existing = (
            db.query(PropertyListing).filter_by(zillow_id=listing.zillow_id).first()
        )
        if not existing:
            db.add(listing)
...
def main():
    ...
    try:
        ...
        with_natural_light = (
            db.query(PropertyListing).filter_by(natural_light_flag=True).count()
        )
        with_outdoor_space = (
            db.query(PropertyListing).filter_by(outdoor_space_flag=True).count()
        )

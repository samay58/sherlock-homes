...
class ScoutService:
    ...
    def create_scout_from_description(self,
                                     user_id: int,
                                     name: str,
                                     description: str) -> Scout:
        """
    ...
    def _parse_scout_description(self, description: str, user_id: int) -> Criteria:
        ...
        criteria = Criteria(
            user_id=user_id,
            name="Auto-generated from Scout",
            is_active=True
        )
        
        # Price parsing
        if "$1,000,000" in description or "$1m" in desc_lower or "1 million" in desc_lower:
            criteria.price_min = 1000000
        if "$2,000,000" in description or "$2m" in desc_lower or "2 million" in desc_lower:
            criteria.price_max = 2000000
        if "$3,000,000" in description or "$3m" in desc_lower or "3 million" in desc_lower:
            criteria.price_max = 3000000
        
        # Bedroom parsing
        if "3 bedroom" in desc_lower or "3 bed" in desc_lower or "three bedroom" in desc_lower:
            criteria.beds_min = 3
        if "4 bedroom" in desc_lower or "4 bed" in desc_lower or "four bedroom" in desc_lower:
            criteria.beds_min = 4
        ...
        # Essential features
        if "natural light" in desc_lower or "sun-drenched" in desc_lower or "bright" in desc_lower:
            criteria.require_natural_light = True
        if "high ceiling" in desc_lower or "vaulted" in desc_lower or "10-foot" in desc_lower:
            criteria.require_high_ceilings = True
        if "outdoor" in desc_lower or "deck" in desc_lower or "patio" in desc_lower or "yard" in desc_lower:
        ...
        if "updated" in desc_lower or "modern system" in desc_lower or "renovated" in desc_lower:
            criteria.require_updated_systems = True
        if "home office" in desc_lower or "workspace" in desc_lower or "work from home" in desc_lower:
        ...
        # Property types
        property_types = []
        if "single-family" in desc_lower or "single family" in desc_lower or "house" in desc_lower:
    ...
    async def run_scout(self, scout_id: int) -> ScoutRun:
        ...
        try:
            ...
            if not criteria:
                ...
                criteria = self.db.query(Criteria).filter(
                    Criteria.user_id == scout.user_id,
            ...
            if new_matches and scout.is_active:
                await self._send_alerts(scout, new_matches[:scout.max_results_per_alert])
                run.alerts_sent = 1
    ...
    def record_feedback(self, scout_id: int, listing_id: str, is_positive: bool):
        ...
        self.db.commit()
        logger.info(f"Recorded {'positive' if is_positive else 'negative'} feedback for listing {listing_id}")
